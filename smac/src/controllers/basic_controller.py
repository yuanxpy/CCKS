...
class BasicMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False,ask_budget=None,agent_obs=None,epi_obs=None,episode_counts=None,obss=None):
        # Only select actions for the selected batch elements in bs
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        if test_mode==False:
            ohter_outputs = self.cal_other_outs(ep_batch, t_ep)
            ohter_colas = self.cal_other_colas(ep_batch, t_ep)
            agent_outputs = self.forward(ep_batch, t_ep, test_mode=test_mode)
            chosen_actions, ask_budget, teacher_scores = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, t_ep, test_mode=test_mode,ask_budget=ask_budget,batch=ep_batch,hidden_states=self.hidden_states,agent_obs=agent_obs,epi_obs=epi_obs,episode_counts=episode_counts,ohter_outputs=ohter_outputs,n_agents=self.n_agents,n_actions=self.n_actions,ohter_colas=ohter_colas,obss=obss)
        else:
            agent_outputs = self.forward(ep_batch, t_ep, test_mode=test_mode)
            chosen_actions, ask_budget, teacher_scores = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, t_ep, test_mode=test_mode,n_agents=self.n_agents)
        
        return chosen_actions, ask_budget, teacher_scores
    ...
    def cal_other_outs(self, ep_batch, t):

        ohter_hidden_states = self.hidden_states.clone()
        agent_inputs = self._build_inputs(ep_batch, t)
        other_outs = []
        dim0, dim1 = agent_inputs.shape
        for i in range(dim0):
            temp_inputs = agent_inputs[i].unsqueeze(0)
            for j in range(dim0-1):
                temp_inputs = torch.cat((temp_inputs,agent_inputs[i].unsqueeze(0)),0) 
            # temp_outs, _ = self.agent(temp_inputs, ohter_hidden_states)
            ohter_hidden_states = self.agent.calc_hidden(temp_inputs, ohter_hidden_states)
            with th.no_grad():
                if self.args.input == 'hidden':
                    latent_state = self.consensus_builder.calc_student(ohter_hidden_states)
                elif self.args.input == 'obs':
                    latent_state = self.consensus_builder.calc_student(temp_inputs)

                # latent_state = latent_state - latent_state.max(-1, keepdim=True)[0].detach()
                latent_state_id = F.softmax(latent_state, dim=-1).detach().max(-1)[1].unsqueeze(-1)
                latent_state_id[ep_batch['alive_allies'][:, t].reshape(*latent_state_id.size()) == 0] = self.args.consensus_builder_dim
                latent_state_embedding = self.embedding_net(latent_state_id.squeeze(-1))
            temp_outs = self.agent.calc_value(latent_state_embedding, ohter_hidden_states)
            other_outs.append(temp_outs)

        return other_outs

    def cal_other_colas(self, ep_batch, t):
        ohter_hidden_states = self.hidden_states.clone()
        agent_inputs = self._build_inputs(ep_batch, t)
        ohter_hidden_states = self.agent.calc_hidden(agent_inputs, ohter_hidden_states)
        with th.no_grad():
            if self.args.input == 'hidden':
                latent_state = self.consensus_builder.calc_student(ohter_hidden_states)
            elif self.args.input == 'obs':
